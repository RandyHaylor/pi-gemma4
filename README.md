...
against a **local gemma4 model served by ollama**, with:

1. **Thinking/reasoning turned OFF** for gemma4 — done *natively inside Pi* via a
   small `before_provider_request` extension that injects the concrete request
   fields gemma4 needs (`reasoning_enabled: false`, `reasoning_effort: "none"`).
   No proxy required.
...
# The CONCRETE per-model payload injection (gemma4's thinking-off levers).
# A model that should keep thinking simply omits this.
export PI_REQUEST_PAYLOAD_INJECTION_JSON='{"reasoning_enabled": false, "reasoning_effort": "none"}'
...
- `reasoning_enabled` alone was **not** sufficient in testing — gemma4 needed
  both `reasoning_enabled:false` and `reasoning_effort:"none"`.
